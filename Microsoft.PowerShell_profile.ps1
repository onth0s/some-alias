...
function global:opf { $target = if ($args.Count -gt 0) { ($args -join ' ') -replace "'" -replace "$([char]96)" } else { Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue; $text = [System.Windows.Forms.Clipboard]::GetText() -replace "$([char]27)\[[\d;]*[a-zA-Z]" -replace '[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]'; foreach ($line in $text -split '\r?\n') { $p = $line -replace "'" -replace "$([char]96)" -replace '^[\s\-*>#]+' -replace '\s*\[[\d\.,\s]+\s*(?:KB|MB|GB)\s*,\s*\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}\]\s*$'; if ($p) { $full = if ([System.IO.Path]::IsPathRooted($p)) { $p } else { $resolved = Resolve-Path -LiteralPath $p -ErrorAction SilentlyContinue; if ($resolved) { $resolved.Path } else { Join-Path (Get-Location).Path $p } }; if ([System.IO.File]::Exists($full) -or [System.IO.Directory]::Exists($full)) { Start-Process $full; Write-Host "opened: $full" -ForegroundColor Green; return } } }; Write-Error "No valid file path in clipboard"; return }; $full = if ([System.IO.Path]::IsPathRooted($target)) { $target } else { $resolved = Resolve-Path -LiteralPath $target -ErrorAction SilentlyContinue; if ($resolved) { $resolved.Path } else { Join-Path (Get-Location).Path $target } }; if ([System.IO.File]::Exists($full) -or [System.IO.Directory]::Exists($full)) { Start-Process $full; Write-Host "opened: $full" -ForegroundColor Green } else { Write-Error "Not found: $full" } }
